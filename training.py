...
def train(model, criterion, optimizer, batch_size):
    ...
    for batch, i in enumerate(range(0, (num_batches-1)*batch_size, batch_size)):
        ...
        inputs = inputs.permute(2, 1, 0).float()   # [18, 16, 50]
        targets = targets.reshape(1, batch_size, seq_len) #[1, 16, 50]
        predictions = model(inputs, targets, src_mask)   #[50,50,1]
        #print(predictions)
...
def evaluate(model, criterion, batch_size):
    ...
    with torch.no_grad():
        ...
        for batch, i in enumerate(range(0, (num_batches-1)*batch_size, batch_size)):
            ...
            inputs, targets = get_batch(valid_seq, valid_label, i, batch_size)
            inputs = inputs.permute(2, 1, 0)  # [18, 256, 50]
            targets = targets.reshape(1, batch_size, seq_len)  #[1, batch size, 50]
